use crate::{lexer_types::{Token, TokenType}, parser_types::ParsedVariableType};
...
pub fn parse_variable_type(tokens: &Vec<Token>, token_pos: &mut usize) -> ReturnT {
    fn scope_to_var(scope: &Scoped) -> ParsedVariableType {
        if scope.types.is_empty() {
            ParsedVariableType::scaler(&scope.name)
        } else {
            ParsedVariableType::generic(&scope.name, scope.types.clone())
        }
    }

    fn parse_tokens(tokens: &Vec<Token>, token_pos: &mut usize, outer: &mut Scoped, is_open_brace: bool, depth: usize) -> ReturnT {
        ...
        while *token_pos < tokens.len() {
            ...
            *token_pos += 1;
            ...
            match &current.token_type {
                ...
                TokenType::NewLine | TokenType::InheritSymbol | TokenType::Equal | TokenType::End => {
                    *token_pos -= 1; 
                    break;
                },
