#[cfg(test)]
pub mod lexer_tests {
    use types_gen::lexer::{ lexer::Lexer, lexer_types::{ Token, TokenType } };

    fn assert_tokens(expected: &Vec<TokenType>, actual: &Vec<Token>) {
        let types: Vec<TokenType> = actual
            .iter()
        ...
        assert!(
            &types == expected,
            "{}",
            format!("Mismatch between parsed, Expected: {:#?}, Actual: {:#?}", expected, types)
        );
    }
